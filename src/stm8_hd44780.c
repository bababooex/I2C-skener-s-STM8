...
void lcd_init(void){
	...
	lcd_command(LCD_CURSOR_OR_DISPLAY_SHIFT | LCD_CURSOR_SHIFT | LCD_SHIFT_RIGHT);
...
uint8_t lcd_bus_read(void){
	uint8_t tmp=0;
	if(GPIO_ReadInputPin(LCD_D4_PORT,LCD_D4_PIN)){tmp |= 1<<0;}
	if(GPIO_ReadInputPin(LCD_D5_PORT,LCD_D5_PIN)){tmp |= 1<<1;}
	if(GPIO_ReadInputPin(LCD_D6_PORT,LCD_D6_PIN)){tmp |= 1<<2;}
	if(GPIO_ReadInputPin(LCD_D7_PORT,LCD_D7_PIN)){tmp |= 1<<3;}
	return tmp;
}
...
void lcd_delay_ms(uint8_t delay){
	uint16_t tmp=10*(uint16_t)delay;
	while(tmp){
	_delay_us(100);
	tmp--;
	}

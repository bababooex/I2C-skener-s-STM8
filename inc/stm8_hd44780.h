...
/*
  ...
  - lcd_init_hw() 
   ...
   16 - Background (K)
	 ...
	 notes about some LCDs
	 - Display DEM16101 (1x16) have strange memory organization. First 8 charaters lays in addresses 0x00 to 0x07, and next 8 characters at addresses 0x40 to 0x47. You cant put single string crossing 8th character.
 */
...
// uncommend if voltage supply is lower then 4.5V
...
#define INIT_WITH_POWERUP_DELAY // in undefined, lcd_init() is called without power up delay (~15ms), usable when application generate powerup delay nonblocking way, by sleep etc.
...
#ifdef LOW_VOLTAGE_MODE
    #define LCD_STARTUP_DELAY1 15 // ms (with VCC >= 4.5V)
#else
    #define LCD_STARTUP_DELAY1 45 // ms (with VCC <4.5V)
#endif
#define LCD_STARTUP_DELAY2 5 // ms
#define LCD_STARTUP_DELAY3 100 // us
#define LCD_E_DELAY 1 // ~us
#define LCD_COMMAND_DELAY 40 // us
...
void lcd_delay_ms(uint8_t delay);											// extending _delay_us() range for lcd_init()
...
/*
Change list:
17.6.2022 	- added lcd_delay_ms() function, because _delay_us have limited range and with higher F_CPU can't realize delays longer then few us
						- corrected delay functions in lcd_init()
18.6.2022		- added macro INIT_WITH_POWERUP_DELAY		
9.7.2022		- modified lcd_delay_ms() function because _delay_us can realise delays only up to 250us (with F_CPU=16000000)
22.10.2022      - added LOW_VOLTAGE_MODE macro, controlling initialisation delay time
*/
